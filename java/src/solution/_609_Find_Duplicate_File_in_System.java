...
public class _609_Find_Duplicate_File_in_System {
  ...
  public List<List<String>> findDuplicate(String[] paths) {
    ...
    Map<String, List<File>> group = files.stream()
        .collect(Collectors.groupingByConcurrent(file -> file.content));
    List<List<File>> duplicateFiles = group.values().stream().filter(list -> list.size() > 1)
        .collect(Collectors.toList());
    return duplicateFiles.stream().map(files1 -> files1.stream().map(
        file -> file.directory + "/" + file.fileName).collect(Collectors.toList()))
        .collect(Collectors.toList());
