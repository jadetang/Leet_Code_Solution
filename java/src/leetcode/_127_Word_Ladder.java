...
public class _127_Word_Ladder implements Medium, Amazon {
   ...
   * reached with "distance" steps; another set wordDict that has not been reached. In the while
   * loop, for each word in the reached set, I give all variations and check if it matches anything
   * from wordDict, if it has a match, I add that word into toAdd set, which will be my "reached"
   * set in the next loop, and remove the word from wordDict because I already reached it in this
   * step. And at the end of while loop, I check the size of toAdd, which means that if I can't
   * reach any new String from wordDict, I won't be able to reach the endWord, then just return 0.
   * Finally if the endWord is in reached set, I return the current steps "distance".
   *
   * The idea is that reached always contain only the ones we just reached in the last step, and
   * wordDict always contain the ones that haven't been reached. This is pretty much what Dijkstra's
   * algorithm does, or you can see this as some variation of BFS.
   */
